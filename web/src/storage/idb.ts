...
export async function getAllProgress(): Promise<ProgressMap> {
  ...
  return migrateProgress(Object.fromEntries(records.map((record) => [record.moduleId, progressValue(record)])));
}
...
export async function replaceModuleProgress(moduleId: string, value: ModuleProgress): Promise<void> {
  await putRecord("progress", { moduleId, ...migrateModuleProgress(clone(value)) });
}
