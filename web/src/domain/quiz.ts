...
const quizCache = new LRUCache<string, QuizQuestion[]>(100);
...
export function parseQuiz(markdown: string, cacheKeyHint?: string): QuizQuestion[] {
  const text = String(markdown || "");
  const cacheKey = cacheKeyHint ? `v2:${cacheKeyHint}` : `v2:${text}`;
