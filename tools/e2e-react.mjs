...
function flowScript() {
  return `(${async function smokeFlow() {
    ...
    const advanceReaderToQuiz = async () => {
      for (let step = 0; step < 6; step += 1) {
        if (document.querySelector(".quiz-card")) return;
        const button = document.querySelector(".reader-forward");
        if (!button) throw new Error("Missing reader forward");
        button.click();
        await delay(300);
      }
    ...
    const passOnboarding = async () => {
      const start = document.querySelector(".onboarding-cta");
      if (!start) return;
      start.click();
      await waitFor(() => document.querySelector(".today-screen"), "Today after onboarding");
    };
    try {
      await passOnboarding();
      const navText = document.querySelector(".topnav")?.textContent || "";
      const navOk = document.querySelectorAll(".topnav a").length === 3 && !navText.includes("Дальше");
      await clickAt(".topnav a", 1);
      await waitFor(() => document.querySelectorAll(".module-tile").length === 24, "Route modules");
      document.querySelector(".module-tile").click();
      await waitFor(() => document.querySelector(".station-screen"), "Block");
      const readingVisible = document.body.textContent.includes("Нутрициология — это наука");
      await advanceReaderToQuiz();
      const quizVisible = document.body.textContent.includes("апельсиновый сок");
      await clickLabel("Профиль");
      ...
      return { ok: navOk && readingVisible && quizVisible && noOverflow, navOk, readingVisible, quizVisible, noOverflow };
    } catch (error) {
...
function mobileScript() {
  return `(${async function mobileSmoke() {
    ...
    const passOnboarding = async () => {
      ...
      start.click();
      for (let attempt = 0; attempt < 50; attempt += 1) {
        if (document.querySelector(".today-screen")) return;
        await delay(100);
      }
      throw new Error("Timed out: Today after onboarding");
    };
    try {
      await passOnboarding();
      const navText = document.querySelector(".topnav")?.textContent || "";
      ...
      document.querySelector(".module-tile").click();
      await delay(500);
      const readingVisible = Boolean(document.querySelector(".station-screen"));
