...
async function main() {
  ...
  try {
    ...
    // reader: open M01 theory
    await drive(`const m01=[...document.querySelectorAll('.module-card')].find(c=>c.querySelector('.mod-id')?.textContent.trim()==='M01');await click(m01);await waitFor(()=>document.querySelectorAll('.lesson-section-card').length>=3);window.scrollTo(0,0);`);
    await delay(400);
