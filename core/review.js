(function (root, factory) {
  ...
  function normalizeReviewItem(item, today = toISODate()) {
    ...
    return {
      ...
      sourceBlock: item.sourceBlock || "theory",
  ...
  function upsertWrongQuestion(review, payload, now = new Date()) {
    ...
    const item = {
      ...
      sourceBlock: payload.sourceBlock || question.sourceBlock || previous.sourceBlock || question?.sourceBlock || "theory",
      mistakeType,
      ...
      reviewStrategy: payload.reviewStrategy || previous.reviewStrategy || "Потренировать понимание в том же формате рассуждения.",
      repair,
  ...
  function itemFromWeakSpot(moduleId, spot, now = new Date()) {
    ...
    return normalizeReviewItem({
      ...
      sourceBlock: spot?.sourceBlock || "theory",
      mistakeType: spot?.mistakeType || "Нужна дополнительная тренировка по этому вопросу",
      diagnosticType: spot?.diagnosticType || diagnosticTypeFrom(spot?.mistakeType),
      userLabel: spot?.userLabel || spot?.mistakeType || "Нужна дополнительная тренировка по этому вопросу",
      shortExplanation: spot?.shortExplanation || spot?.repair || "",
      reviewStrategy: spot?.reviewStrategy || "Потренировать понимание в том же формате рассуждения.",
